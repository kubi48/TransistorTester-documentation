...
\begin{figure}[H]
\centering
 \includegraphics[width=.93\textwidth]{../GNU/Cesr2CZ.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=1.\textwidth]{../GNU/Elco_esrCZ.pdf}
...
Jako reference se zde nejlépe odhaduje. Křivka \inquotes{Multimeter} zobrazuje odchylky,
které mají a Peaktech ~ 3315 multimetru.
Další křivka \inquotes{LCR} zobrazuje odchylky naměřené měřičem Peaktech ~ 2170 LCR v nejpříznivějším
kmitočtovém rozsahu.
Pro srovnání, křivka \inquotes{ATmega168as} také ukazuje odchylky měření testeru vybaveného ATmega168.
...
Tuto nulovou hodnotu jsem zjistil při použití verze desky \inquotes{DG2BRS V 5.2.1}.
...
Body \inquotes{diff2ref} ukazují rozdíl napětí, který vznikne po nabíjení kondenzátoru s \(660nF\) na
...
Z obrázků \ref{fig:mega168all}, \ref{fig:mega168Aall} a \ref{fig:mega168PAall} ukazují body \inquotes{CapErr} 
odhadované chyby měření jednotlivých vzorků ATmega v promile.
Je nápadné, jak se body \inquotes{CapErr} řídí body \inquotes{diff2ref}.
Proto body \inquotes{diff}  ukazují rozdíl mezi příslušnými body \inquotes{CapErr} a \inquotes{diff2ref}.
Při střední hodnotě pro \inquotes{diff} lze tedy udělat dobrý odhad korekce měření kondenzátoru z rozdílu
...
Makefile \inquotes{PARTNO = } musela být, vzhledem k AVRDUDE programu,
upravena na jiný typ procesoru (\inquotes{m168}, \inquotes{m168p}, \inquotes{m328} nebo \inquotes{m328p}) upravena.
...
Body s koncovým znakem \inquotes{unc} zobrazují odchylky bez kalibrace.
Řádky končící na \inquotes{cal} ukazují odchylky stejných procesorů
